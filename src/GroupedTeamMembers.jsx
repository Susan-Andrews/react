...
function groupTeamMembers(){
  ...
  function handleTeamClick(event){
    var transformedGroupData=groupedEmployees.map((groupedData) => groupedData.team === event.currentTarget.id ? {...groupedData,collapsed:!groupedData.collapsed}
      :groupedData);
     ...
     <hr/>
                {
                  item.members.map(member => {
                    return (
                      ...
                      <span className="text-dark">Full Nmae: {member.fullName}</span> </h5>
      <p>Designation:{member.designation}</p></div>
                    )
                  })
                }
